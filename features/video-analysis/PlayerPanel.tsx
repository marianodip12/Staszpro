/**
 * PlayerPanel — Collapsible player roster for the Video Analyzer.
 * Ported 1:1 from the original Handball-analizador.
 */

import { useState, type FormEvent } from 'react';
import { Users, Plus, Trash2 } from 'lucide-react';
import type { VideoPlayer } from '@/domain/video-events';
...
export const PlayerPanel = ({ players, onAdd, onRemove }: PlayerPanelProps) => {
    ...
    <div className="rounded-2xl bg-[#0d1117] border border-[#21262d] overflow-hidden">
      ...
      {open && (
        <div className="px-4 pb-4 flex flex-col gap-3">
          ...
          {/* Player list */}
          {players.length === 0 ? (
            <p className="text-[#484f58] font-mono text-xs text-center py-2">
              Sin jugadores. Agregá uno arriba.
            </p>
          ) : (
            <div className="flex flex-col gap-1 max-h-36 overflow-y-auto">
              {players.map((p) => (
                <div
                  key={p.id}
                  className="group flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-[#161b22] border border-[#21262d]"
                >
