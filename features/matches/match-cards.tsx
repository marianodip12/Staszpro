...
export const MatchCard = ({ match, myTeamName, onAnalyze, onViewEvolution, onVideo, hasVideoAccess, onDelete }: MatchCardProps) => {
  const t = useT();
          ...
          {onVideo && (
            <Button
              ...
              className={cn(
                'flex-1 gap-1',
                hasVideoAccess
                  ? 'bg-goal/15 border-goal/30 text-goal hover:bg-goal/20'
                  : 'opacity-70',
              )}
              title={hasVideoAccess ? 'Análisis con video' : 'Disponible en Club / Elite'}
            >
              <VideoIcon />
