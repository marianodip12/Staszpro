...
const LiveMatchPagePro = () => {
  ...
  // Borrar localmente Y soft-delete server-side: ambos en uno, para que la UI
  // borre el evento y al mismo tiempo Supabase lo deje oculto. Sin esta capa,
  // el cliente borraba local pero el sync re-bajaba el evento en el próximo ciclo.
  ...
  const handleShotOutcomePicked = (outcome: ShotOutcome, situation: import('@/domain/types').Situation | null) => {
    if (!pendingShot) return;
    setPendingShot({
      ...pendingShot,
      step: 'shooter',
      outcome,
      draft: { ...pendingShot.draft, situation },
    });
  };
