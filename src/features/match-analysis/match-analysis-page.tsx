...
interface MatchAnalysisPageProps {
  /** Si se pasa, usa este match en vez de buscarlo en el store. Ideal para vista compartida. */
  externalMatch?: import('@/domain/types').MatchSummary;
  /** Si está en true, oculta los botones de edición y compartir. */
  readonly?: boolean;
}
...
export const MatchAnalysisPage = ({ externalMatch, readonly: readonlyProp = false }: MatchAnalysisPageProps = {}) => {
  ...
  const [editingNewEvent, setEditingNewEvent] = useState<HandballEvent | null>(null);
  ...
  return (
    <div className="max-w-7xl mx-auto w-full px-4 md:px-6 lg:px-8 space-y-3 pb-4">
      ...
      {/* Arco */}
      <ProSectionLock
        title="Cuadrante del arco"
        description="Mirá a qué cuadrante del arco fueron los tiros. Identificá patrones del arquero rival y de tus jugadores."
        disabled={readonly}
      >
        <section>
      ...
      {/* Cancha — lanzamientos */}
      <ProSectionLock
        title="Mapa de tiros por zona"
        description="Visualizá desde dónde se hicieron los tiros: extremo, lateral, pivote, 7m. Eficacia por zona y heatmap."
        disabled={readonly}
      >
        <section>
          <div className="flex items-center justify-between mb-1.5">
        ...
        {showEditPanel && (
          <>
            <p className="text-[11px] text-muted-fg">
              Tocá un evento para editarlo, o agregá uno nuevo. Los cambios se guardan automáticamente y el resultado se recalcula.
            </p>
            <button
              type="button"
              onClick={() => {
                // Crear evento "en blanco" para agregar
                ...
                setEditingNewEvent(newEvent);
              }}
              className="w-full py-2.5 rounded-md border border-dashed border-primary/40 bg-primary/5 hover:bg-primary/10 text-primary text-xs font-medium transition-colors"
            >
      ...
      {/* Dialog para crear evento nuevo (reusa el mismo edit dialog) */}
      <EventEditDialog
        open={!!editingNewEvent}
        onClose={() => setEditingNewEvent(null)}
        event={editingNewEvent}
        homeName={match.home}
        awayName={match.away}
        onSave={(patch) => {
          if (!editingNewEvent) return;
          addCompletedEvent(match.id, { ...editingNewEvent, ...patch } as HandballEvent);
          setEditingNewEvent(null);
        }}
        onDelete={() => setEditingNewEvent(null)}
      />
    </div>
  );
};
